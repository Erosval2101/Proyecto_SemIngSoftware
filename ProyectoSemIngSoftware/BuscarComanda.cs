...
{
    public partial class BuscarComanda : Form
    {
        String id_mesero;
        public BuscarComanda(String id)
        {
            InitializeComponent();
            id_mesero = id;
        }
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            buscar.CommandText = ("select * from comanda where IDComanda = '" + txtID.Text + "'");
        ...
        private void btnModificar_Click(object sender, EventArgs e)
        {
            ...
            buscar.CommandText = ("select * from comanda where IDComanda = '" + txtID.Text + "' and (Estatus = 'Pedida' or Estatus = 'Asignada')");

            MySqlDataReader leerConsulta = buscar.ExecuteReader();
            if (leerConsulta.Read())
            {
                String id_comanda = this.dtgvComanda.CurrentRow.Cells[0].Value.ToString();
                ModificarComanda ver = new ModificarComanda(id_comanda, id_mesero);
                ver.Show();
                this.Hide();
            }
            else
            {
                MessageBox.Show("No se puede modificar la comanda", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            conectar.Close();
        }
        ...
        private void btnCancelar_Click(object sender, EventArgs e)
        {
            ...
            if (leerConsulta.Read())
            {
                ...
                String query = "update comanda set Estatus = 'Cancelada' where IDComanda = '" + id_comanda + "'";

                MySqlCommand registrar = new MySqlCommand(query, conectar);

                if (registrar.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Comanda cancelada exitosamente");
                    dtgvComanda.Rows.Clear();
                    dtgvDetalle.Rows.Clear();
                }
                else
                {
                    MessageBox.Show("Error al cancelar la comanda", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
        ...
        private void btnCerrarComanda_Click(object sender, EventArgs e)
        {
            ...
            buscar.CommandText = ("select * from comanda where IDComanda = '" + txtID.Text + "' and Estatus = 'Preparada'");
            ...
            if (leerConsulta.Read())
            {
                ...
                String query = "update comanda set Estatus = 'Cerrada' where IDComanda = '" + id_comanda + "'";
                ...
                if (registrar.ExecuteNonQuery() == 1)
                {
                    MessageBox.Show("Comanda cerrada exitosamente");
                    dtgvComanda.Rows.Clear();
                    dtgvDetalle.Rows.Clear();
                }
                else
                {
                    MessageBox.Show("Error al cerrar la comanda", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
